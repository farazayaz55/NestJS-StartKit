...
describe('ArticleService', () => {
  ...
  describe('Update Article', () => {
    it('should throw UnauthorizedException if user is not allowed to update', async () => {
      ctx.user = { id: 1, roles: [ROLE.USER], username: 'baduser' };
      mockedRepository.getById.mockResolvedValue(new Article());

      await expect(
        service.updateArticle(ctx, 1, { title: 'new', post: 'post' }),
  ...
  describe('deleteArticle', () => {
    ...
    it('should throw UnauthorizedException if user is not allowed to delete', async () => {
      ...
      article.author = { id: 2 } as any;

      mockedRepository.getById.mockResolvedValue(article);

      await expect(service.deleteArticle(ctx, 1)).rejects.toThrow(
